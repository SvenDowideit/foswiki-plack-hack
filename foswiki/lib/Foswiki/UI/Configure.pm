...
foreach my $module ( 'FindBin', 'File::Spec', 'Config', ) {
    Foswiki::UI::Configure::_loadBasicModule($module);
}
...
my $setlib = File::Spec->catfile( @root, 'foswiki', 'bin', 'setlib.cfg' );
pop(@root);
...
sub render_configure_ui {
    Foswiki::render_configure_ui();
}
...
$time = time();
$url = $query->url();
$action = $query->param('action') || 'Configure';
...
sub _dispatchContents {
    my $stub =  Foswiki::Configure::Item->new();
...
# displayStatus  - 1 = No Changes,  2 = Changes,  4 = No Extensions, 8 = Extensions, 16 = Email Test
    ...
    if ( $transact eq 'UPDATE' ) {
        my $valuer = Foswiki::Configure::Valuer->new( $Foswiki::defaultCfg,
            \%Foswiki::cfg );
        my %updated;
        ...
        foreach my $key ( sort keys %updated ) {
            ...
            my $type        = $query->param("TYPEOF:$key");
            $valueString = ( '*' x 15 ) if ( $type eq 'PASSWORD' );
...
#    $root->addChild($cgi_checker);
    ...
    $ui->{controls} =  Foswiki::Configure::GlobalControls->new();
    ...
    if ( !$isFirstTime && !$Foswiki::cfg{Password} ) {
        $html .=
          Foswiki::Configure::UI::getTemplateParser()->readTemplate('sanitypw');
    }
